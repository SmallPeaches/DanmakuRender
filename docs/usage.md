...
### B站直播强制原画录制（实验性功能）
B站现在对一些观看人数较多的主播开启了强制二压的功能，隐藏了原画直播流。这些直播间网页端的原画实际上是蓝光画质，表现为流链接里含有`bluray`或者`prohevc`等字样，或者视频统计信息的encoder字段中有`hevc_n`或者`h264_n`，而不是只有`libobs xxx`。默认情况下录制会录制网页端的最高画质，也就是二压后的直播流。       

但是目前发现可以通过一些手段绕过限制并录制主播推流的真实原画，在配置文件中需要设置如下字段：
```yaml
download_args:
  ...
  # 使用pyrequests录制引擎
  ...
  ...
```
按这样设置后，程序将会强制录制原画，无论获取的流地址原来是何种格式和分辨率。**需要特别提醒的是，强制原画功能不一定生效，并且可能录制失败且不会自动转为非原画录制**，建议仅对保证二压的主播使用此功能。
