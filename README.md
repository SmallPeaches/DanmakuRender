...
- `--margin` 指定弹幕行距，默认16
- `--font` 指定弹幕字体，这里要求输入为字体文件的位置，默认为Windows系统里微软雅黑的文件位置(C:\Windows\Fonts\msyhbd.ttc)
- `--fontsize` 指定弹幕字体大小，默认为24
...
- `--opacity` 指定弹幕不透明度，默认为0.9
#### 其他参数
- `--debug` 使用debug模式，将录制信息输出到stdout
- `-v` 查看版本号
